...
$(document).ready(function(){
  ...
  $("#letters").change(function(){
    if($(this).val()=="ALL"){
      ...
      $.getJSON("<?php echo base_url();?>pages_controller/view_coachLetter/"+$(this).val(),success=function(data){
        var value="No Records To Display";
        for(var i=0;i<data.length;i+=2){
          value+='<tr onclick="document.location =\''+'<?php echo base_url();?>pages_controller/view_coach/'+data[i]+'\'"><td>'+data[i+1]+'</td><td>Coach</td></tr>';
        }
        $("#tbody").html("");
        $("#tbody").append(value);
      });
