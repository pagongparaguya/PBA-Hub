...
$(document).ready(function(){
  ...
  <?php $tid = $this->page_model->get_playerTeam($player->PLAYER_ID);
    ...
    cur+="<tr onclick=\"window.location ='<?php echo base_url().'pages_controller/view_player/'.$player->PLAYER_ID;?>'\"><td style='text-align: center;' class='table-content'><img class='table-img' src='<?php echo base_url();?>assets/img/player/<?php echo $player->PLAYER_PORTRAIT_PHOTO?>' width='70'/> <?php echo $player->PLAYER_FULLNAME;?></td><td style='text-align: center;' class='table-content'><img class='table-img' src='<?php echo base_url();?>assets/img/team/<?php echo $team->TEAM_LOGO?>' width='70'/><?php echo $team->TEAM_NAME;?></td></tr>";
  <?php endforeach;?>
  $("#tbody").append(cur);
