...
$(document).ready(function(){
  ...
  <?php foreach($products as $product):
    ...
    users+="<tr><input type='hidden' class='prod_id' value='<?php echo $product->PROD_ID;?>'/><td class='table-content hide'><?php echo $product->PROD_ID;?></td><td class='table-content productName' onclick=\"window.location ='<?php echo base_url().'auction_controller/view_product/'.$product->PROD_ID;?>'\"><img src='<?php  echo $product->IMAGE1;?>' width='70'/><br/><?php echo $product->PROD_NAME;?></td><td class='table-content' id='username'><img src='<?php echo $user->USER_IMAGE;?>' width='70'/><br/><?php echo $user->USERNAME;?></td><td class='table-content' id='pType'><?php echo $product->PROD_CAT;?></td><td class='table-content' id='pStatus'><?php echo $product->PROD_STAT;?></td><td><?php echo $product->START_BID;?></td><td><button class='button alert tiny del'>X</button></td></tr>";
  <?php endforeach;?>
  $("#tbody").append(users);
