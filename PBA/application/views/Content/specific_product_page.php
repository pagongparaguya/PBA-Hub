      ...
      <ul class="team-row small-block-grid-1 medium-block-grid-1 large-block-grid-1 teams">
        ...
        <h1>IMAGES:</h1>
          <div class="imageProduct">
            <div><img style="border:1px solid blue;padding:5px;" width="400" src="<?php echo $product->IMAGE1;?>" alt="1-portrait" /></div>
            <div><img style="border:1px solid blue;padding:5px;" width="400" src="<?php echo $product->IMAGE2;?>" alt="2-portrait" /></div>
            <div><img style="border:1px solid blue;padding:5px;" width="400" src="<?php echo $product->IMAGE3;?>" alt="3-portrait" /></div>
            <div><img style="border:1px solid blue;padding:5px;" width="400" src="<?php echo $product->IMAGE4;?>" alt="4-portrait" /></div>
            <div><img style="border:1px solid blue;padding:5px;" width="400" src="<?php echo $product->IMAGE5;?>" alt="5-portrait" /></div>
          </div>
        </li>
      </ul>
  </div>
    ...
    <div class="row row-content">
      <div class="large-12 columns" style="text-align:center;">
          ...
          if($this->auction_model->getProductInfoWithUserId($product->PROD_ID,$userInfo->USER_ID)){
            ...
            <?php }else{//pending?>
              <button class="button tiny round startBid" data-reveal-id="changeStatOnModal">Start Bid?</button>
            <?php }?>
          <?php 
            }else{
            if($product->PROD_STAT=='Closed'){?>
  ...
  <div class="row row-content">
    <ul class="team-row small-block-grid-1 medium-block-grid-2 large-block-grid-2 teams">
      <li>
        <p>Sold by: <a href="<?php echo base_url().'account_controller/view_otherUser/'.$user->USER_ID;?>"><?php echo $user->USERNAME;?></a></p>
    ...
    <div class="row">
      <div class="large-12 columns">
        ...
        <form action="<?php echo base_url();?>auction_controller/addBid" method="post">
          ...
          <div class="addBidPanel">
          </div>
        </form>
        <?php }?>
        <h1 class="current-teams bid-block" style="background-color:#4A5052;color:white;">PRODUCT BIDS</h1>
      </div>
    </div>
    <div class="row row-content">
      <div class="large-12 columns">
  ...
  <!-- END PRODUCT BID-->
   ...
   <!--- START PRODUCT COMMENT-->
    <div class="row">
      <div class="large-12 columns">
        <h1 class="current-teams comment-block" style="background-color:#4A5052;color:white;">PRODUCT COMMENTS</h1>
      </div>
    </div>
  ...
  <div id="changeStatOnModal" class="reveal-modal tiny" data-reveal>
    <div class="small-8 large-centered columns">
      <input type="type" name="prodIdOnModal" id="prodIdOnModal"/>
      <center> 
        <h5>Changes Status of <b id="prodOnName"></b> to 'On-going'?</h5> 
  ...
  <div id="changeStatClModal" class="reveal-modal tiny" data-reveal>
    <div class="small-8 large-centered columns">
      <input type="type" name="prodIdClModal" id="prodIdClModal"/>
      <center> 
        <h5>Changes Status of <b id="prodClName"></b> to 'Closed'?</h5> 
    ...
    $(".addBid").click(function(){
      $.ajax({
        ...
        success: function(data, status) {
          ...
          $(".addBidPanel").html("");
          $(".addBidPanel").append("<div id='bids'><input type='number' name='bidValue' required placeholder='Your Bid' min='"+max+"'/><button class='round' type='submit'>Submit</button><button class='alert round cBid'>Cancel</button></div>");
        }
      });
    });
